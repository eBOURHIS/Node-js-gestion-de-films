...
function AfficheSerie(id){
      let url = ''.concat(baseURL, 'tv/', id, '?api_key=', key , '&language=fr');
      //fetch(url).then(result=>result.json()).then((data)=>{
      fetch(url).then(result=>result.json()).then((data)=>{
      str = JSON.stringify(data, null, 4);
      obj = JSON.parse(str);
      ...
      document.getElementById("sortie").innerHTML = obj.first_air_date;
      ...
      document.getElementById('description').innerHTML = obj.overview;
      document.getElementById('episode').innerHTML = "Épisodes : "+obj.number_of_episodes;
      document.getElementById('season').innerHTML = "Saisons : "+obj.number_of_seasons;
      document.getElementById("duration").innerHTML = "Durée : "+obj.episode_run_time[0]+" min";
      document.getElementById('Episodes').value = obj.number_of_episodes;
      document.getElementById('Saisons').value = obj.number_of_seasons;

      for(var i = 0; i<obj.created_by.length; i++)
      {
        document.getElementById('creator').innerHTML += " "+obj.created_by[i].name+",";
      }

      document.getElementById("idTMDB").value = obj.id;
      document.getElementById("Titre").value = obj.name;
      document.getElementById("Image").value = img;
      document.getElementById("DateParution").value = obj.first_air_date;

      document.getElementById("Description").value = obj.overview;
      document.getElementById("Duree").value = obj.episode_run_time[0];
...
function getCredS(id){
      let url = ''.concat(baseURL, 'tv/', id, '/credits?api_key=', key , '&language=fr');
      //fetch(url).then(result=>result.json()).then((data)=>{
      fetch(url).then(result=>result.json()).then((data)=>{
      str = JSON.stringify(data, null, 4);
      obj = JSON.parse(str);
      for(var i = 0; i<10; i++)
      {
        document.getElementById('actors').innerHTML += " "+obj.cast[i].name+",";
        document.getElementById('Acteur').value += " "+obj.cast[i].name+",";
      }
      for(var i = 0; i<obj.crew.length; i++)
      {
        if(obj.crew[i].job == "Creator"){
          document.getElementById('creator').innerHTML += " "+obj.crew[i].name+",";
          document.getElementById('Realisateur').value += " "+obj.crew[i].name+",";
        }
      }
  })
}
